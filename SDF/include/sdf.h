...
/**
 @brief Reads the metadata for the next block in the SDF file

 This routine reads and parses the metadata for the next block in the file.
 If the block pointer is currently set the next block is found and read. If the block pointer is
 NULL the first block is read.
